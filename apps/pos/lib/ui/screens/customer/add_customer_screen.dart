...
class AddCustomerScreen extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _AddCustomer();
}

class _AddCustomer extends State<AddCustomerScreen> {
  ...
  @override
  void initState() {
    selectedCountry = customerController.countries[0];
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)!.add_new_customer),
      ),
      body: createForm(),
    );
  }
  ...
  void handleCreateButtonClick() async {
    ...
    var newCustomerInfo = Customer(-1, nameControler.text, streetControler.text, "0", cityControler.text,taxControler.text, selectedCountry?.id, 0, phoneControler.text, postCodeControler.text, "0", emailControler.text, barcodeControler.text, phoneControler.text, "", "unsynced");
    var result = await customerController.addNewCustomer(newCustomerInfo, context);
